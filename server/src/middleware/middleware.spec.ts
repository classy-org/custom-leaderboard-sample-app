...
describe('tokenExpirationMiddleware', () => {
  ...
  it('should call next() if token is valid', async () => {
    ...
    await middleWareModule.tokenExpirationMiddleware(mockRequest, mockResponse, next);
  ...
  it('should fetch a new token if token is expired', async () => {
    ...
    jest
      ...
      .mockReturnValue(
        Promise.resolve({ data: { access_token: 'new_token' } }) as Promise<AxiosResponse<authModule.AuthResponse, any>>
      );
  ...
  it('should handle error when fetching token fails', async () => {
    ...
    jest.spyOn(authModule, 'getAccessToken').mockRejectedValue(new Error(errorMessage));
  ...
  it('should wait for token refresh if token is currently refreshing', async () => {
    ...
    setTimeout(() => {
      jest.spyOn(middleWareModule, 'waitForTokenRefresh').mockReturnValue(Promise.resolve(true));
    }, 100);
